...
resolve_port() {
  ...
  if [[ -n "${OLLAMA_INTERCEPTOR_PORT:-}" ]]; then
    printf '%s' "$OLLAMA_INTERCEPTOR_PORT"
    return 0
  fi
  ...
  if [[ -f ".env" ]]; then
    while IFS= read -r raw_line; do
      ...
      local value="${line#*=}"
      key="$(trim "$key")"
      if [[ "$key" == "OLLAMA_INTERCEPTOR_PORT" ]]; then
...
echo "Stopping ollama-interceptor"
...
for pid in "${listening_pids[@]}"; do
  ...
  if [[ "$command_line" == *"codex_openai_ollama_proxy.ollama_interceptor"* ]]; then
    matching_pids+=("$pid")
  else
...
if (( ${#matching_pids[@]} == 0 )); then
  echo "[ERROR] Found listener(s) on port $PORT_TO_STOP, but none appear to be ollama-interceptor. Refusing to stop unrelated processes."
...
while (( SECONDS < deadline )); do
  ...
  if (( still_running == 0 )); then
    echo "[OK] Interceptor stopped cleanly."
    exit 0
  fi
...
if [[ "${FORCE:-0}" == "1" ]]; then
  echo "[WARN] Interceptor still running after ${SHUTDOWN_WAIT_SECONDS}s. Sending SIGKILL because FORCE=1 was set."
  kill -9 "${still_running_pids[@]}"
  echo "[OK] Interceptor force-stopped."
  exit 0
fi

echo "[WARN] Interceptor still running after ${SHUTDOWN_WAIT_SECONDS}s: ${still_running_pids[*]}"
